 ...
 */
export async function handleCreateStudentSubmit({ state, user, addDocument,
    setButtonText, response, dispatch }: handleCreateStudentSubmit): Promise<void> {

    if (user === null) {
        return;
    }

    if (state.submittingAuthor.trim().length === 0 ||
        state.submittingAuthor.split(/\w\w+/).length < 2) {
        dispatch({ type: SubmissionAction.Error, payload: 'Please enter a full name (i.e., First and Last)' });
        return;
    } else if (state.posterTitle.trim().length === 0 ||
        state.posterTitle.split(/\w\w+/).length < 2) {
        dispatch({ type: SubmissionAction.Error, payload: 'Please enter a full title (i.e., 3+ Words)' });
    } else if (state.posterAbstract.trim().length === 0 ||
        state.posterAbstract.split(/\w\w+/).length > 120) {
        dispatch({ type: SubmissionAction.Error, payload: 'Check abstract length (e.g., over 120 words)' });
        return;
    }
