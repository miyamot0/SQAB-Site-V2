...
function costFunctionNoise(inputs, delay) {
  return inputs[0];
}
function costFunctionExponential(inputs, delay) {
  return Math.exp(-Math.exp(inputs[0]) * delay);
}
function costFunctionHyperbolic(inputs, delay) {
  return 1 / (1 + Math.exp(inputs[0]) * delay);
}
function costFunctionBetaDelta(inputs, delay) {
  return InvIt(inputs[0]) * Math.pow(InvIt(inputs[1]), delay);
}
function costFunctionGreenMyerson(inputs, delay) {
  return 1 / Math.pow(1 + Math.exp(inputs[0]) * delay, Math.exp(inputs[1]));
}
function costFunctionRachlin(inputs, delay) {
  return Math.pow(1 + Math.exp(inputs[0]) * Math.pow(delay, Math.exp(inputs[1])), -1);
}
function costFunctionGeneralizedHyperbolic(inputs, delay) {
  return Math.pow(1 + delay * Math.exp(inputs[0]), -(Math.exp(inputs[1]) / Math.exp(inputs[0])));
}
function costFunctionEbertPrelec(inputs, delay) {
  return Math.exp(-Math.pow(Math.exp(inputs[0]) * delay, Math.exp(inputs[1])));
}
function costFunctionBeleichrodt(inputs, delay) {
  return InvIt(inputs[2]) * Math.exp(-Math.exp(inputs[0]) * Math.pow(delay, Math.exp(inputs[1])));
}

/* Invert Transform */
function InvIt(value) {
  return Math.exp(value) / (Math.exp(value) + 1);
}

/* Calculate and assign Bayes Factor */
function CalculateBF(result, noiseBic) {
  result.BF = Math.exp(-0.5 * (result.BIC - noiseBic));
}

/* Calculate and assign Probability */
function CalculateProbabilities(result, bayesSum) {
  result.Probability = result.BF / bayesSum;
}
...
function integrate(func, start, end, stepSize) {
  ...
  for (var x = start; x < end; x += stepSize) area += func(x + stepSize / 2) * stepSize;
...
function CalculateAUC(obj, lowerX, upperX) {
  ...
  if (obj.Model == 'Noise') {
    ...
    fitter = function (x) {
      return Math.exp(-obj.Params[0] * x);
    };
  else if (obj.Model == 'Hyperbolic')
    fitter = function (x) {
      return 1 / (1 + obj.Params[0] * x);
    };
  else if (obj.Model == 'Beta-Delta')
    fitter = function (x) {
      return obj.Params[0] * Math.exp(-(1.0 - obj.Params[1]) * x);
    };
  else if (obj.Model == 'Green-Myerson')
    fitter = function (x) {
      return 1 / Math.pow(1 + obj.Params[0] * x, obj.Params[1]);
    };
  else if (obj.Model == 'Rachlin')
    fitter = function (x) {
      return Math.pow(1 + obj.Params[0] * Math.pow(x, obj.Params[1]), -1);
    };
  else if (obj.Model == 'Loewstein-Prelec')
    fitter = function (x) {
      return Math.pow(1 + x * obj.Params[0], -obj.Params[1] / obj.Params[0]);
    };
  else if (obj.Model == 'Ebert-Prelec')
    fitter = function (x) {
      return Math.exp(-Math.pow(obj.Params[0] * x, obj.Params[1]));
    };
  else if (obj.Model == 'Beleichrodt')
    fitter = function (x) {
      return obj.Params[2] * Math.exp(-(obj.Params[0] * Math.pow(x, obj.Params[1])));
    };
...
function CalculateAUClog10(obj, lowerX, upperX) {
  ...
  if (obj.Model == 'Noise') {
    ...
    fitter = function (x) {
      return Math.exp(-obj.Params[0] * Math.pow(10, x));
    };
  else if (obj.Model == 'Hyperbolic')
    fitter = function (x) {
      return 1 / (1 + obj.Params[0] * Math.pow(10, x));
    };
  else if (obj.Model == 'Beta-Delta')
    fitter = function (x) {
      return obj.Params[0] * Math.exp(-(1.0 - obj.Params[1]) * Math.pow(10, x));
    };
  else if (obj.Model == 'Green-Myerson')
    fitter = function (x) {
      return 1 / Math.pow(1 + obj.Params[0] * Math.pow(10, x), obj.Params[1]);
    };
  else if (obj.Model == 'Rachlin')
    fitter = function (x) {
      return Math.pow(1 + obj.Params[0] * Math.pow(Math.pow(10, x), obj.Params[1]), -1);
    };
  else if (obj.Model == 'Loewstein-Prelec')
    fitter = function (x) {
      return Math.pow(1 + Math.pow(10, x) * obj.Params[0], -obj.Params[1] / obj.Params[0]);
    };
  else if (obj.Model == 'Ebert-Prelec')
    fitter = function (x) {
      return Math.exp(-Math.pow(obj.Params[0] * Math.pow(10, x), obj.Params[1]));
    };
  else if (obj.Model == 'Beleichrodt')
    fitter = function (x) {
      return obj.Params[2] * Math.exp(-(obj.Params[0] * Math.pow(Math.pow(10, x), obj.Params[1])));
    };
