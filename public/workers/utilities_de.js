...
function CalculatePmax(result) {
  if (result.Model === 'Zero-bounded Model (with K)') {
    lamba = function (x) {
      var lowerLimit = ihsTransform(result.Q0) - result.K;
      var demand =
        ihsTransform(result.Q0) + result.K * (Math.exp(-result.Alpha * result.Q0 * x) - 1);
    ...
    lamba = function (x) {
      var demand =
        ihsTransform(result.Q0) *
        Math.exp(-(result.Alpha / ihsTransform(result.Q0)) * result.Q0 * x);
      var work = unIHS(demand) * x;

      return -Math.log10(work);
    };

    var result2 = numeric.uncmin(lamba, [1], 1e-30, undefined, 2000, undefined, undefined);
...
function GetBestAgent() {
  return populationAgents[lowestLossIndex];
}

/* Get best population agent cost */
function GetBestCost() {
  return populationAgentsCost[lowestLossIndex];
}
...
function Optimize(costFunc) {
  ...
  for (var i = 0; i < maxits; i++) ProcessGeneration(costFunc, boundsU.length);
}
...
function ProcessGeneration(costFunc, nParams) {
  ...
  for (var x = 0; x < populationSize; x++) {
    ...
    for (var i = 0; i < nParams; i++)
      z[i] =
        populationAgents[a][i] + mutationFactor * (populationAgents[b][i] - populationAgents[c][i]);

    var R = random.integer(0, nParams - 1);

    var r = new Array(nParams);

    for (var i = 0; i < nParams; i++) r[i] = random.real(0, 1);

    var newX = new Array(nParams);

    for (var i = 0; i < nParams; i++) {
      if (r[i] < crossoverRate || i == R) newX[i] = z[i];
      else newX[i] = populationAgents[x][i];
    }
